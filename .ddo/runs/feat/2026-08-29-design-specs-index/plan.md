...
> revision: 2 ｜ 文档模式: single ｜ 依据: `run://.ddo/runs/feat/2026-08-29-design-specs-index/spec.md`（revision 2）
> rev2 变更：新增「git push 前沉淀提示」AI 流程规则（FR-PUSH-1 / AC-8 / PD-6 → DEC-6）。

---

## 执行摘要

将根目录 DESIGN.md（445 行、未入 git 的样式规则唯一权威）拆分重组为 `design/` 规范索引目录：1 份整体索引（`design/DESIGN.md`）+ 9 份组件样式规则文件 + 9 份基础规则文件；根目录新增 `AGENTS.md` 作为 AI 理解仓库的入口，形成「AGENTS.md → design/DESIGN.md → 规则文件」的三层加载结构。`AGENTS.md` 同时承载新增的 AI 流程规则：`git push` 前提示用户是否沉淀本次需求的组件样式变更，确认后按 git 变更清单整理沉淀到样式架构规则。同步修正 CLAUDE.md / README.md / CONTRIBUTING.md 中指向旧路径（`../ddo-design/DESIGN.md` 与根 `DESIGN.md`）的权威来源描述。全过程不改变任何视觉取值，样式数值以 `src/styles/tokens.css` 为唯一数据来源。
...
| 范围内 | 根目录新增 `AGENTS.md`（仓库逻辑 + 分层导航入口 + 开发流程规则） | FR-AGENTS-1、FR-PUSH-1 |
...
| 范围外 | 不实现自动化 push 拦截（无 git hooks / CI 门禁）；流程规则以文档约束 AI 行为 | FR-PUSH-1 边界 |
...
flowchart TD
    ...
    E --> F
    B --> G[开发流程规则：<br/>git push 前沉淀提示]
    G -.用户确认沉淀.-> H[git 查看本次变更]
    H -.整理写回.-> D
```

- 入口层只讲「这个仓库是什么、规范在哪、怎么读、推送前要做什么」，不复制规则内容。
- 索引层承载设计哲学摘要、文件地图（文件 → 管辖范围 → 关键 token）、迭代守则与裁决记录。
- 规则层每份文件自包含单一组件/领域的完整规则，取值一律引用 token 变量名。
- 沉淀闭环：push 前提示 → 用户确认 → `git diff`/`git log` 盘点样式变更 → 按归属写入规则文件（如需新取值，同步 tokens.css 并记录）。
...
| 沉淀规则落 AGENTS.md | accepted | AI 开发流程属入口层职责，与 DEC-4 分治一致；人类与各类 AI 均可见 | DEC-6 |
| 沉淀规则落 pre-push hook | rejected | 需要代码与安装步骤，强制拦截超出用户「提示」语义 | DEC-6 |
...
| DEC-6 | 「git push 前沉淀提示」规则归 `AGENTS.md`「开发流程」节，完整描述三要点：① `git push` 前提示用户是否沉淀本次需求的组件样式变更；② 用户确认后通过 `git diff`/`git log` 盘点本次样式变更；③ 将变更整理沉淀到 `design/` 对应规则文件（涉及新取值时同步 `tokens.css` 并在索引迁移记录留痕）。`design/DESIGN.md` 迭代守则仅放一行指针，不复述全文 | 与 DEC-4 分治一致（AI 工作流归入口层）；「提示」语义用文档约束即可，不引入钩子代码 | `AGENTS.md`、`design/DESIGN.md` | 用户要求钩子化或改归属 | PD-6、FR-PUSH-1 |
...
|---|---|---|---|
| `design/DESIGN.md` | 整体索引：总览、文件地图、迭代守则（含沉淀规则单行指针）、裁决与迁移记录 | 新增（由根 DESIGN.md 重构） | FR-DESIGNMD-1、DEC-6 |
...
| `AGENTS.md` | 入口层：仓库逻辑与分层导航；「开发流程」节承载 git push 前沉淀提示规则 | 新增 | FR-AGENTS-1、FR-PUSH-1 |
...
| 流程规则效力 | 沉淀规则为文档约束，无强制拦截；AI 未读 AGENTS.md 时规则不生效——通过分层入口（AGENTS.md 首层）最大化触达 | DEC-6 |
...
| 沉淀规则成文：AGENTS.md 含 git push 前提示 → 用户确认 → git 盘点变更 → 写回规则文件的完整流程描述；design/DESIGN.md 有指针 | 文本核对三要点 | AC-8 |
...
| PD-6 沉淀规则归属 | AGENTS.md「开发流程」节承载全文，design/DESIGN.md 迭代守则放单行指针（DEC-6） | DEC-6 |
...
- **规则空转风险**：沉淀规则无强制机制，可能被跳过。缓解：规则置于入口层首屏动线（开发流程节），并写明沉淀的检查动作；后续如需强制可升级为钩子（本次范围外）。
...
- ✅ **同意**：批准当前 revision（2），进入 **Test-Planning**。
